...
contract dBank {
  ...
  function deposit() payable public {
    ...
    require(msg.value >= 10000000000000000, "Error, deposit must be >= 0.1 Ether");
  ...
  function withdraw() public {
    ...
    uint _userBalance = etherBalanceOf[msg.sender];

    //check user's hodl time
    uint _depositTime = block.timestamp - depositStart[msg.sender];

    //calc interest per second
    uint _interestPerSecond = 31668017 * (etherBalanceOf[msg.sender] / 1e16);
