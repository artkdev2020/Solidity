...
contract Color is ERC721, ERC721Enumerable {
    ...
    function mint(string memory _color) public onlyOwner {
        ...
        coinsCount++;
        // create new coin
        ColorCoin memory _coin = ColorCoin(
            coinsCount,
            0,
            _color,
            msg.sender,
    ...
    function buy(uint256 _id, bool _newStatus) public {
        require(msg.sender != ownerOf(_id));
        ColorCoin memory _coin = coins[_id];
        require(_coin.isForSale != _newStatus);

        _coin.isForSale = _newStatus;
        _coin.owner = msg.sender;
        coins[_id] = _coin;
    }
    ...
    function transfer(address payable _owner, uint256 _id) public payable {
        // Require unique color
        ColorCoin memory _coin = coins[_id];
        ...
        coins[_id] = _coin;
        // event
        emit Sale(_id, msg.value, _owner);
    }
